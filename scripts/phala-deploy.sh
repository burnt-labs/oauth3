...
# Check critical variables — secrets only (non-secret config is hardcoded
# in docker-compose.phala.yml so it's covered by TEE attestation).
REQUIRED_VARS=(
  "COOKIE_KEY_BASE64"
...
# ── Generate env file (secrets only) ───────────────────────────────────
